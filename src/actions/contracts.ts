  ...
  try {
    ...
    const contract = await prisma.contract.create({
      data: {
        ...
        startDate: new Date(parsed.startDate + (parsed.startDate.includes('T') ? '' : 'T12:00:00Z')),
        endDate: parsed.endDate ? new Date(parsed.endDate + (parsed.endDate.includes('T') ? '' : 'T12:00:00Z')) : null,
